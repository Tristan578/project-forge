...
- `QuickStartDialog.tsx` — the "Make me a game" modal entry point into the game-creation pipeline (desktop CTA + `MobileToolbar`'s `Sparkles` icon-only trigger on compact widths). Opens `OrchestratorPanel` before starting a run and claims `quickStartGateOwner` so the two don't both render `ApprovalGateDialog` for the same gate (PF-1215). "Plan my game" only designs (`startQuickStart` stops at `awaiting_approval`); a plan review then shows the plan and its cost, and only its "Build it" calls `runPipelineFromPlan`, which is where the build's tokens are reserved (the reserve POST carries no abort signal: an aborted fetch discards the reply while the server still deducts). "Close" keeps the plan (reopening returns to the review); "Discard plan" cancels it; a build refused before any step ran (reservation declined, or no engine) returns the plan to `awaiting_approval` with the reason in `orchestratorError`, which the review shows (with "Buy tokens" to `/settings?tab=tokens` for a short balance) and which survives closing the dialog and in-app navigation but not a full page load such as Stripe checkout (#10270); only the route's documented refusals (400/401/402/403/429) count as 'nothing spent'; a reservation whose outcome is unknown (no reply, any 5xx, unreadable 2xx) goes to 'failed' with `RESERVATION_UNCONFIRMED_MESSAGE` instead, because the hold may already have been taken; every run clears `orchestratorError` when it starts (#6831)
