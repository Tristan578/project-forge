...
- `physics/physics2dPayload.ts` — The ONLY place the browser and engine 2D physics vocabularies meet, in both directions (`buildSetPhysics2dPayload` / `buildUpdatePhysics2dPayload` / `parsePhysics2dWire`). Payloads are built key-by-key from `PHYSICS2D_PATCH_FIELDS`, not annotated with `satisfies` — excess-property checking never applies to spread properties. Also owns `defaultPhysics2dData()` and both enum variant tables, pinned against `engine/src/core/physics_2d.rs` textually by its test (PF-1167). `physics/updatePhysicsPayload.ts` is the 3D counterpart
