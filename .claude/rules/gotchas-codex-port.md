...
- **Codex has TWO edit channels.** A patch can arrive as tool `apply_patch`, or carried in a shell command, which hooks see as `Bash`. Codex intercepts exactly two shell forms (`apply_patch <<'EOF'` and `cd <path> && apply_patch <<'EOF'`, sole top-level statement), and the adapter's mode `edit` recognises exactly those — widening it to "any command containing `*** Begin Patch`" blocked a plain `grep` for that string, and cutting the envelope with a regex let an added line saying `*** End Patch` hide every later file. The generator wires `PreToolUse` Edit/Write hooks for both channels; `PostToolUse` cannot see the second one at all (that call carries no command), and the exec tool's `workdir` is never visible to any hook. A new blocking check on file edits belongs on `PreToolUse`, and an `if` on such a hook must be a `Bash(…)` condition or the generator stops.
