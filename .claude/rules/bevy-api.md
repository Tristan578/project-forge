...
- Local fork at `.transform-gizmo-fork/` patched for Bevy 0.19 (#8887): `render.rs` builds its pipeline in `RenderStartup`, sources the colour format from the view key, and anchors its `Transparent3d` sort at the camera to keep the 0.18 draw order; `lib.rs` scopes the `AssetMut` guard. Its egui-family pins (0.34) did not move.
