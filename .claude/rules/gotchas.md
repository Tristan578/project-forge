...
- **Single-root-lockfile monorepo: a `web/package.json` bump WITHOUT regenerating the ROOT lockfile breaks `npm ci` on main.** There is exactly ONE `package-lock.json` (repo root); `web/package-lock.json` does not exist. Dependabot's npm updater is configured `directory: /web`, so it edits `web/package.json` but **cannot** touch the root lockfile → the manifest range stops matching the pinned lockfile version → every `npm ci` (all of CI + the Quality Gates jobs that run it as setup, e.g. Rust Security Audit, Lighthouse) fails with `EUSAGE Missing: <pkg>@<ver> from lock file`. Shipped twice (#8655, #8658 → #8683). A caret bump is enough: `^0.12.0`→`^0.13.0` resolves to the newest satisfying version, which the stale lockfile doesn't carry. **Detection:** `Lockfile Sync` gate (`.github/workflows/lockfile-sync.yml` → `scripts/check-lockfile-sync.sh`) regenerates the lockfile from the manifests and fails the PR on any diff — deterministic and pre-merge, unlike `npm ci` which only trips after the bad state lands. **Fix any drift** by running `npm install --package-lock-only` from the repo root and committing the updated `package-lock.json`. **After merging ANY dependency PR, verify `npm ci` against `origin/main`** — a Dependabot force-push can leave the last green run reflecting an intermediate (consistent) state, not the final manifest-only squash.
