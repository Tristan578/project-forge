...
echo ""
echo "--- Notes ---"
echo "  'stripe' is on ^22.6.0 (API version 2026-08-26.dahlia) — the SDK pins its ApiVersion literal, so any major/minor bump must update apiVersion in stripe-client.ts + the 3 billing route tests + the webhook route comment (see references/version-pins.md)"
echo "  'wasm-bindgen' Rust dep is pinned at =0.2.127 — cannot upgrade without full WASM rebuild"
