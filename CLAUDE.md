...
- `PUBLISH_TO_R2` — mirrors a published game's scene data into R2 as a JSON bundle on `POST /api/publish` and serves `/play/[userId]/[slug]` from that CDN-backed object on read, falling back to Postgres `projects.sceneData` when absent or on any R2 error (#7580, PF-212). **This is the one flag in this list that is NOT default-off:** `isPublishToR2Enabled()` in `web/src/lib/config/assetStorage.ts` DEFAULTS ON whenever `ASSET_BUCKET_NAME` is configured (a deploy with R2 credentials starts serving bundles with no second flip) and off otherwise (local dev / preview without a bucket). An explicit `PUBLISH_TO_R2=false` always wins (kill the mirror without pulling the bucket env); an explicit `PUBLISH_TO_R2=true` forces it on before a bucket is wired (the write then fails open). Any value other than the exact strings `"true"`/`"false"` (case-insensitive, trimmed) falls through to the bucket-presence default. The bundle url produced when `CDN_URL` is set is tracked ONLY in the `cdn_bundle_key` column; `published_games.cdn_url` always stays the `/play` route so the community gallery's Play/Share links resolve to the playable page. Check this flag first when a published game unexpectedly starts or stops resolving from the CDN bundle vs. Postgres. Storage module: `web/src/lib/storage/publishedGameStorage.ts`.
...
- **Four generated-artifact sync gates can break `main` if you edit a source without regenerating** — the root `package-lock.json` (single-root lockfile), gh-aw `*.lock.yml`, `docs/api/openapi.json`, and `apps/docs/data/capability-matrix.json` (edit `docs/capability-matrix.md`, then `npm run sync:capability-matrix` from the repo root — otherwise `check-manifest-sync.ts` and `capabilityMatrix.test.ts` both go red). Full failure modes, detection gates, and the exact fix recipe for each: `.claude/rules/gotchas-build-ci.md` → Build & CI.
