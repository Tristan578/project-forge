...
- **Sentry/Copilot re-review on every push** — Each push to a PR triggers a fresh review. Fix ALL issues in ONE commit, run `security-reviewer` agent BEFORE pushing, verify with `next build`. Goal: one push per review cycle.
- **`sanitizePrompt().filtered` without `.safe` check** — `.filtered` can be non-null when `.safe` is false (contains filtered version of unsafe content). Always check `.safe` first: `s.safe ? s.filtered! : 'fallback'`. Never use `s.filtered ?? rawString`.
- **Never spread LLM objects into engine commands** — Allowlist fields with `Number.isFinite()` guards. `...cameraConfig` passes `__proto__`, `Infinity`, arbitrary keys. Use explicit key iteration over allowed params.
- **`VERCEL_ENV` includes `'development'`** — Truthy check (`process.env.VERCEL_ENV || ...`) catches `vercel dev`. Use explicit `=== 'production' || === 'preview'` for deployed-only features.
- **Vitest 4.x jest-dom matcher registration** — `import '@testing-library/jest-dom/vitest'` side-effect doesn't work in 4.1.2+. Use explicit `import { expect } from 'vitest'; import * as matchers from '@testing-library/jest-dom/matchers'; expect.extend(matchers)` in vitest.setup.ts. Component tests need `@vitest-environment jsdom` directive.
- **R2 engine CDN requires manual upload** — WASM files are gitignored, not deployed with Vercel. Upload via `wrangler r2 object put spawnforge-engine/latest/<variant>/<file> --file <path> --remote`. Verify: `curl -s -o /dev/null -w "%{http_code}" https://engine.spawnforge.ai/latest/engine-pkg-webgpu/forge_engine.js`.
