...
//     and passes mode `edit`: on a Bash payload such a hook does nothing unless
//     the command carries a `*** Begin Patch` envelope, in which case the patch
//     is checked exactly as if it had arrived through the patch tool.
//     (PostToolUse cannot do the same: there the intercepted call carries no
//     command at all. That gap is recorded in the support matrix.)
...
function main() {
  ...
  // A patch carried inside a shell command (see the header): everything from
  // the envelope's first line to its last.
  const carried = toolName === 'Bash' && typeof input.tool_input?.command === 'string'
    ? /\*\*\* Begin Patch[\s\S]*?\*\*\* End Patch/.exec(input.tool_input.command)?.[0]
    : undefined;
  if (editMode && toolName === 'Bash' && !carried) process.exit(0); // an ordinary command: nothing for a file hook to see
  if (toolName === 'apply_patch' || (editMode && carried)) {
    const patch = carried ?? input.tool_input?.command ?? input.tool_input?.input ?? input.tool_input?.patch;
