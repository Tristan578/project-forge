...
import { readCapabilityMatrix } from '../../lib/capabilityMatrix';
 ...
 *
 * Rendered from `data/capability-matrix.md`, the in-root copy of the canonical
 * `docs/capability-matrix.md`. The two are pinned byte-identical by
 * `web/src/lib/config/__tests__/capabilityMatrix.test.ts`, which also fails
 * when a generation capability or manifest category has no row, so this page
  ...
  return (
    <main style={{ maxWidth: '72rem', margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      {doc ? (
        ...
        // what the file says — so an unreadable copy is an explicit notice.
        <p role="alert" style={{ color: 'var(--foreground, #fafafa)' }}>
          The capability matrix could not be read from this deployment. The canonical copy is at{' '}
