...
const LENGTH_TOKEN_KEYS = new Set([
    '--sf-radius-sm', '--sf-radius-md', '--sf-radius-lg', '--sf-radius-xl', '--sf-radius-full',
    '--sf-border-width',
]);
const FONT_TOKEN_KEYS = new Set(['--sf-font-ui', '--sf-font-mono']);
const DURATION_TOKEN_KEYS = new Set(['--sf-transition']);
...
export function validateCustomTheme(input, options) {
    ...
    if (name.length > 64)
        return { ok: false, error: 'Theme name must be 64 characters or fewer.' };
    if (author.length > 64)
        return { ok: false, error: 'Author must be 64 characters or fewer.' };
    if (description.length > 256)
        return { ok: false, error: 'Description must be 256 characters or fewer.' };
    // Tokens
    const rawTokens = typeof obj.tokens === 'object' && obj.tokens !== null
        ? obj.tokens
        : {};
