...
export function useTheme(options) {
    ...
    const resolveTheme = useCallback(() => {
        if (projectTheme && VALID_THEMES.has(projectTheme))
            return projectTheme;
        return readTheme();
    }, [projectTheme]);
    const [theme, setThemeState] = useState(resolveTheme);
    const [effectsEnabled, setEffectsEnabledState] = useState(readEffects);
    useEffect(() => {
    ...
    const setEffectsEnabled = useCallback((enabled) => {
