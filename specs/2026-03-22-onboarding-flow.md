...
5. Blocking and accessibility: a step with no `actionRequired` (every step of this tour, and the welcome/congratulations cards of the others) blocks the page. The backdrop and highlight ring take pointer events, pointer events aimed outside the bubble are stopped in the capture phase (so a control stacked above the ring, or the click Enter synthesises on a focused button, cannot fire either), and Tab/Shift+Tab cycle within the bubble; the bubble is then `aria-modal="true"`. A step WITH `actionRequired` behaves as before: backdrop and ring are `pointer-events: none`, the click reaches the highlighted control, Tab is not trapped and the dialog is non-modal. The bubble is a `role="dialog"` named by the step title and described by the step text and a visible key hint (Right arrow next, Left arrow back, Esc skip). Every step moves focus to Next (or to the dialog while Next is disabled), a polite status region announces each new step, and focus returns to whatever had it before the tour (the Help button) when the tour ends. The header X is a 44px target labelled "Skip tutorial".
