...
2. `TUTORIAL_CAPABILITIES` in `web/src/data/tutorials.ts` is highlight-only: an intro card, then the quick-start trigger (`[data-testid="quick-start-trigger"]`), Play (`[data-testid="play-controls-play"]`) and Export (`[data-testid="scene-toolbar-export"]`), in that order. The targets are dedicated test ids because `aria-label="Play"` also matches the Adaptive Music inspector's preview button, and a paused editor shows Resume. No step has `actionRequired`, so the tour makes no request, dispatches no engine command and does not change the scene.
3. A step whose target is not rendered (the compact layout has no Export control) shows as an untargeted card, and Next still advances. The Export step tells compact users they can ask the AI chat to export an HTML file or a zip instead, and that this is a chat message, so it uses tokens (`/api/chat` charges up front; the chat's `export_game` cannot produce embed code).
4. Bubble placement (`placeBubble` in `TutorialOverlay.tsx`, used by every tutorial): the bubble is at most 400px wide with 16px viewport margins and re-lays out on window resize and rotation, including untargeted cards. A targeted bubble sits 16px from its target on the step's preferred side, and flips to the other side of a 'top'/'bottom' step when the preferred side has less than 380px of room (the compact quick-start trigger is docked at the bottom, so its bubble goes above it). Above a target the bubble is anchored by its bottom edge, below it by its top edge, and max-height caps it at the room on that side (it scrolls if longer), so it does not overlap the target or leave the viewport at any real height. A 'left'/'right' step with no room beside its target goes below it instead. Only when neither side of the target has 160px of room does the step fall back to a centred card capped to the viewport, which may cover the target.
5. Blocking and accessibility: blocking is an explicit opt-in, `blocksPage: true` on the `TutorialFlow`, set only by this tour. On such a tutorial a step with no `actionRequired` (every step of this tour) blocks the page. It is deliberately not inferred from a missing `actionRequired`: the older tutorials' closing cards have none and say "Press Stop to return to Edit Mode" while the engine is still in play mode, so they must keep the Stop button reachable. The backdrop and highlight ring take pointer events, pointer events aimed outside the bubble are stopped in the capture phase (so a control stacked above the ring, or the click Enter synthesises on a focused button, cannot fire either), and Tab/Shift+Tab cycle within the bubble; the bubble is then `aria-modal="true"`. Every step of every other tutorial, and a step WITH `actionRequired` in any tutorial, behaves as before: backdrop and ring are `pointer-events: none`, the click reaches the highlighted control, Tab is not trapped and the dialog is non-modal. The bubble is a `role="dialog"` named by the step title and described by the step text and a visible key hint (Right arrow next, Left arrow back, Esc skip). Every step moves focus to Next (or to the dialog while Next is disabled), a polite status region announces each new step, and focus returns to whatever had it before the tour (the Help button) when the tour ends. The header X is a 44px target labelled "Skip tutorial".
6. The tour sets `completesOnboarding: false`. `OnboardingChecklist` auto-dismisses only when a tutorial that counts as onboarding completes (`tutorialCompletesOnboarding`), so finishing this tour leaves the checklist visible. Escape skips the tour and records nothing, so it can be started again.
