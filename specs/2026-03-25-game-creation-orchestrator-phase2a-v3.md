# Game Creation Orchestrator -- Phase 2A Spec (v3)

> **Status:** DRAFT -- Awaiting Approval
> **Date:** 2026-03-25
> **Revision:** v3 (fixes 12 issues introduced in v2)
> **Ticket:** TBD
...
| ND2 | DX | File count recounted honestly: 21 lib files, 12 fixtures, 8 test files = 41 total |
...
export interface ExecutorContext {
  ...
  store: EditorStore;               // [FIX: NS3] Full store ref for reading state
  projectType: '2d' | '3d';
  userTier: UserTier;
  signal: AbortSignal;
...
// [FIX: NS3] EditorStore type imported from store for type-safety
...
export async function decomposeIntoSystems(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    const sanitizedRefGames = data.feelDirective.referenceGames.map(
      game => {
        const gameResult = sanitizePrompt(game, 100);
        // [FIX: NS2] Check .safe before using .filtered -- same pattern as constraints
        return gameResult.safe
          ? (gameResult.filtered ?? game.slice(0, 100))
          : game.slice(0, 100);
      }
    );
    ...
    return {
      ...
      feelDirective: {
        ...
        mood: sanitizedMood.safe
          ...
          : data.feelDirective.mood.slice(0, 100),
  ...
  const approvalGates: ApprovalGate[] = [
    {
      ...
      afterStepId: 'step_0',         // After planning is presented (before execution)
      status: 'pending',
      displayData: {
