...
**Update (2026-09-24, #8887):** unblocked by bevy_rapier 0.35.0 (2026-07-12, `bevy ^0.19.0`) and executed by PF-935 / #8887 — bevy 0.19.1, bevy_rapier 0.35, bevy_hanabi 0.19, bevy_panorbit_camera 0.35, fork re-patched. The checklist below is historical; what the migration actually needed beyond it (light `shadow_maps_enabled`, `AssetMut`, the gizmo fork's `RenderStartup` pipeline, a rustc 1.95 floor) is recorded in `.claude/rules/bevy-api.md` → "Bevy 0.19 changes".
...
| wasm-bindgen | =0.2.127 (pinned) | 0.2.x | `^0.2.108` required by wgpu 29 | **PIN COMPATIBLE** |
...
`wgpu 29.0.3` (the version required by bevy_render 0.19) declares `wasm-bindgen ^0.2.108`. **The pin does not need to change for a bevy 0.19 migration.**

The pin recorded when this audit was written was `=0.2.108`. It has since moved to `=0.2.127`, out of band and with explicit user approval (PF-DEP / #9380) — `0.2.127` satisfies `^0.2.108` exactly as `0.2.108` did, so the conclusion above is unchanged in either direction. The pin should still only move with explicit user approval per the engine architecture rules, and the crate must always match the installed `wasm-bindgen-cli`.
...
- [ ] **wasm-bindgen: no change needed for this migration.** The pin is now `=0.2.127` (moved out of band by PF-DEP / #9380, superseding the `=0.2.108` this audit recorded); it satisfies wgpu 29's `^0.2.108` requirement either way.
...
- [x] WASM binary size within the CI gate: the "Check WASM binary sizes" step in `quality-gates.yml` (`WASM Build` job) budgets all four packages. 0.19.1 grew them 15-20%, so the budgets were raised to 26/27/25/26 MiB (webgl2/webgpu/webgl2-runtime/webgpu-runtime, +10% tolerance) through `docs/operations/wasm-size-budgets.md`, owner-approved 2026-09-26; the record is under that doc's "Budget history".
...
wasm-bindgen lock pin: `=0.2.108` — verified unchanged by this branch. (The pin has since moved to `=0.2.127` under PF-DEP / #9380; this line records what was measured on 2026-07-02, not the current pin.)
