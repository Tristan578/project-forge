...
## 9. API & MCP Documentation

### 9.1 Rationale

The design system site should be the canonical technical reference for both design patterns AND API/MCP specifications. Co-locating them means agents working on or with SpawnForge find everything in one place — component library, API contracts, MCP commands, all under the same domain.

### 9.2 Public vs Internal Documentation

| Content | Location | Visibility |
|---------|----------|-----------|
| Design system (tiers 1+2) | `design.spawnforge.ai` | Public |
| MCP command reference | `design.spawnforge.ai/mcp` | Public (read-only docs for integrators) |
| Public API reference (Swagger/OpenAPI) | `design.spawnforge.ai/api` | Public |
| Internal API docs (admin routes, billing internals) | Internal build only | Vercel Deployment Protection |
| Design system (tier 3) | Internal build only | Vercel Deployment Protection |

### 9.3 Implementation

The Storybook site gains additional doc pages (MDX) alongside component stories:

```
apps/design/
  stories/
    primitives/          <- component stories
    composites/          <- component stories
    tokens/              <- token reference
    effects/             <- ambient effects
    docs/                <- MDX documentation pages
      mcp-reference.mdx  <- MCP command catalog (generated from commands.json)
      api-reference.mdx  <- OpenAPI/Swagger spec rendered
      api-internal.mdx   <- Internal routes (gated by INCLUDE_INTERNAL)
...
Every documentation page includes a footer:
...
- **No AI provider attribution.** No "Built with Claude", "Powered by Anthropic", "Generated by OpenAI", or similar. No AI provider logos, badges, or links.
- **No AI tool attribution.** No "Generated with Claude Code", "Co-Authored-By" AI trailers, or AI tool marketing.
- **Exception:** If a specific AI provider's branding guidelines REQUIRE attribution when using their API (e.g., "Powered by X" in API responses), that attribution goes only where legally required, never on the docs site or in commit messages.
- **SpawnForge branding only.** The site represents SpawnForge as a product by Tristan Nolan. Period.
...
- [ ] MCP command reference auto-generated from commands.json on design site
- [ ] Public API reference rendered on design site
- [ ] Internal API docs gated behind INCLUDE_INTERNAL
- [ ] All doc pages show "Last updated" with git timestamp + author
- [ ] No AI provider/tool attribution anywhere on the site
