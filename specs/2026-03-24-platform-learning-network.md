...
Layer 2: Pattern Extraction (batch, offline)
  - Cluster successful scenes by system composition (e.g., movement+camera+challenge profiles)
...
3. **`web/src/lib/analytics/publishSignal.ts`** -- Capture scene snapshot at publish time.
   ...
   - Attach metadata: system-composition tags, entity count, command count to create, user tier.
...
4. **`web/src/lib/db/schema.ts`** -- New tables.
   ...
   scene_recipes: id, game_id (FK published_games), system_tags (text[]), entity_types (text[]),
...
6. **`web/src/app/api/cron/extract-patterns/route.ts`** -- Daily cron job.
   ...
   - Cluster by system-composition profile (e.g., movement:walk+jump + camera:side-scroll + challenge:physics).
...
7. **`web/src/lib/db/schema.ts`** -- Pattern library table.
   ```
   pattern_library: id, system_profile, pattern_type (entity_composition | parameter_distribution | command_sequence),
...
8. **`web/src/lib/chat/recipeInjector.ts`** -- Context enrichment.
   - Before sending chat message to LLM, detect intent (system composition, feature type).
...
9. **`web/src/lib/chat/templateSuggester.ts`** -- Smart template ranking.
   ...
   - Use keyword overlap + system-composition matching from pattern library.
