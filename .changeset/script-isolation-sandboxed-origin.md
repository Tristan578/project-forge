...
Browser support: in CI the sandboxed transport runs scripts in Chromium and Firefox, and does not run them in WebKit (Safari). There the worker fails to start, so it fails closed: Play stops with that message and nothing is sent, and Safari users cannot run game scripts at all with the flag on. Do not enable the flag for production users until WebKit support exists.
