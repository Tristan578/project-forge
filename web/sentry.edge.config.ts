...
if (DSN) {
  Sentry.init({
    ...
    integrations: [
      ...
      // Anthropic SDK call running on the edge runtime. Input/output recording
      // is disabled in production to avoid capturing PII.
      Sentry.anthropicAIIntegration({
        recordInputs: !IS_PROD,
        recordOutputs: !IS_PROD,
      }),
      // Captures AI SDK (Vercel AI) spans: model name, token usage, latency,
      // and tool call traces for edge runtime streamText/generateText calls.
