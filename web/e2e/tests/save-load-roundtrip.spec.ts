...
test.describe('Save/Load round-trip — store level @ui', () => {
  ...
  test('inject entities → serialize scene → deserialize → entities match', async ({ page, editor }) => {
    ...
    await page.evaluate((sceneJson: string) => {
      ...
      const scene = JSON.parse(sceneJson) as {
        entities: Array<{ entityId: string; name: string; entityType: string }>;
      };
  ...
  test('entity properties survive round-trip (position [5,10,15])', async ({ page, editor }) => {
    ...
    await page.evaluate(
      (args: { entityId: string; name: string; entityType: string }) => {
        const store = (window as unknown as Record<string, unknown>).__EDITOR_STORE as {
          getState: () => {
      ...
      { entityId: entity.entityId, name: entity.name, entityType: entity.entityType },
    );
