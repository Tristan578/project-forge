/**
 * pixelLayers — native multi-layer data model for the Pixel Art Editor.
 *
 ...
 * its own pixel grid, plus visibility and opacity. All functions here are pure:
 * they never mutate their inputs, so they compose cleanly with React state and
 * with undo/redo snapshots.
 *
...
export interface Layer {
  ...
  visible: boolean;
  /** Layer-wide opacity multiplier, clamped to 0..1. */
  opacity: number;
...
/* ─── Stack operations (all return new arrays) ───────────────────────────── */
