...
export function welcomeEmail(userName: string): { subject: string; html: string } {
  ...
  const html = baseLayout(`
    ...
    ${p(`Hi ${highlight(userName)}, we&apos;re thrilled to have you on board.`)}
...
export function paymentFailed(userName: string): { subject: string; html: string } {
  ...
  const html = baseLayout(`
    ${h1("Payment Failed")}
    ${p(`Hi ${highlight(userName)}, we were unable to process your latest payment.`)}
...
export function tokenBalanceLow(
  ...
  const html = baseLayout(`
    ...
    ${p(`Hi ${highlight(userName)}, your AI token balance is running low.`)}
