 ...
 *
 * `spawn_entity.id` is the only entry today. An MCP client driving the engine
 * directly has a legitimate use for supplying its own entity id, but the chat path
 * does not: `sceneGraphSlice.spawnEntity` mints the id itself and returns it
 * synchronously, so a model-supplied id buys nothing. Worse, the engine's
 * `is_valid_override_id` validates only length and control characters — an id that
 * collides with an existing entity is accepted, and every id-matching loop in the
 * engine then addresses the wrong entity.
 */
