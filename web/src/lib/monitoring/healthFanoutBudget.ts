 ...
 *
 * Rendering a health report costs six outbound network calls — Neon, the
 * engine CDN, Clerk (`HEAD api.clerk.com/v1/jwks`, sending `CLERK_SECRET_KEY`),
 * the chat backend, Upstash (a read-only EVAL, billed per command) and Stripe
 * (`GET /v1/balance`, sending `STRIPE_SECRET_KEY`, credit-free — #9719). Three
 * public surfaces can trigger that fan-out: the `/health` page,
