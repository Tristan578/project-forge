...
describe('command counts quoted across the repo', () => {
  ...
  const docs: CountedDoc[] = [
    ...
    },
    {
      ...
      claims: [`${chatToolCount} of the ${total}`, `permits ${bridgeCount} of the ${total} command names`],
    },
    {
    ...
    },
    {
      ...
      claims: [`all ${total} registered MCP commands`],
    },
 ...
 *
 * The count is pinned as well as the names, so adding a gate means coming here:
 * the fifth (the Codex CLI surface generated by tools/agentic-sync/port.mjs,
 * #9745) was first added to CLAUDE.md alone, and this test is what said so.
 */
describe('the capability-matrix copy is registered as a generated-artifact sync gate', () => {
  ...
  it('CLAUDE.md counts five artifacts and names every one of them', () => {
    const line = claudeMd
      .split(/\r?\n/)
    ...
    expect(line).toContain('Five generated-artifact sync gates');
    for (const artifact of [
      'package-lock.json',
      '*.lock.yml',
      'docs/api/openapi.json',
      'apps/docs/data/capability-matrix.json',
      // The Codex CLI surface: every file `port.mjs --write` regenerates.
      '.agents/skills/',
      '.codex/agents/',
      '.codex/hooks.json',
      '.codex/hook-conditions.json',
      'tools/agentic-sync/port.lock.json',
    ]) {
  ...
  it('the fix recipe carries the Codex surface and its regen command too', () => {
    expect(recipe).toContain('node tools/agentic-sync/port.mjs --write');
    expect(recipe).toContain('scripts/check-codex-port.sh');
  });
});
