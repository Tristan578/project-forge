...
export function handleTransformEvent(
  ...
  switch (type) {
    ...
    case 'SCENE_GRAPH_UPDATE': {
      ...
      store.setFullGraph(payload);
      store.recomputeLightState(payload);
    ...
    case 'SCENE_NODE_REMOVED': {
      ...
      const store = useEditorStore.getState();
      store.removeNode(payload.entityId);
      if (payload.components) {
        store.onLightNodeRemoved(payload.components);
      }
      useEditorStore.setState({ sceneModified: true });
      scheduleAutoSave();
      return true;
