...
describe('audioSlice', () => {
  ...
  describe('Reverb zones', () => {
    ...
    it('should remove reverb zone', () => {
      store.getState().setReverbZone('entity-1', reverbData, true);
      expect(store.getState().reverbZones['entity-1']).toEqual(reverbData);
  ...
  describe('Audio Snapshots', () => {
    it('should save snapshot and store in Zustand', () => {
      ...
      const state = store.getState();
      expect(state.audioSnapshots['calm']).toMatchObject({ name: 'calm', crossfadeDurationMs: 500 });
    ...
    it('should save snapshot with default crossfade duration', () => {
      ...
      const state = store.getState();
      expect(state.audioSnapshots['default-dur']).toMatchObject({ name: 'default-dur', crossfadeDurationMs: 1000 });
    });
    ...
    it('should delete snapshot from Zustand and audioManager', () => {
      ...
      expect(store.getState().audioSnapshots['temp']).toMatchObject({ name: 'temp' });
