...
export function setCommandBatchDispatcher(dispatcher: BatchCommandDispatcher | undefined): void {
  ...
  _dispatchCommandBatch = (commands) => {
    ...
    const response = dispatcher(commands);
    commands.forEach(({ command }, index) => {
      // A partially failed batch may still have applied other commands.
      if (response.results[index]?.success !== false) observeCaptureCommand(command);
    });
    return response;
  };
}
