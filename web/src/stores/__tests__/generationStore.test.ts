...
describe('generationStore', () => {
  ...
  describe('clearCompleted', () => {
    it('should remove completed jobs', () => {
      ...
      const state = useGenerationStore.getState();
      expect(state.jobs.job1).toBeUndefined();
      expect(state.jobs.job2).toMatchObject({ id: 'job2', status: 'processing' });
    });
    ...
    it('should remove failed jobs', () => {
      ...
      const state = useGenerationStore.getState();
      expect(state.jobs.job1).toBeUndefined();
      expect(state.jobs.job2).toMatchObject({ id: 'job2', status: 'pending' });
    });
    ...
    it('should keep pending, processing, and downloading jobs', () => {
      ...
      const state = useGenerationStore.getState();
      expect(state.jobs.job1).toMatchObject({ id: 'job1', status: 'pending' });
      expect(state.jobs.job2).toMatchObject({ id: 'job2', status: 'processing' });
      expect(state.jobs.job3).toMatchObject({ id: 'job3', status: 'downloading' });
  ...
  describe('hydrateFromServer', () => {
    ...
    it('should merge hydrated jobs with existing in-memory jobs', async () => {
      ...
      expect(jobs['local-1']).toMatchObject({ id: 'local-1' });
    });
  });
});
