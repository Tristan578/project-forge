...
describe('GET /api/keys', () => {
  ...
  it('returns 403 if tier is starter', async () => {
    ...
    vi.mocked(assertTier).mockReturnValue(NextResponse.json({ error: 'TIER_REQUIRED' }, { status: 403 }));
  ...
  it('returns configured providers for eligible tiers', async () => {
    ...
    const mockProviders: Array<{ provider: Provider; createdAt: Date }> = [
      { provider: 'openai', createdAt: new Date('2026-01-01T00:00:00Z') },
    ];
    vi.mocked(listConfiguredProviders).mockResolvedValue(mockProviders);
