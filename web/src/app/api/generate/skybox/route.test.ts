...
function makeRequest(body: unknown): NextRequest {
  return new NextRequest('http://test/api/generate/skybox', {
    method: 'POST',
...
describe('POST /api/generate/skybox', () => {
  beforeEach(() => {
    ...
    vi.mocked(MeshyClient).mockImplementation(
      function (this: InstanceType<typeof MeshyClient>) {
        this.createTextToTexture = vi.fn().mockResolvedValue({ taskId: 'task-1' });
  ...
  it('returns 400 for invalid JSON', async () => {
    const req = new NextRequest('http://test/api/generate/skybox', {
      method: 'POST',
      body: 'not json',
    });
  ...
  it('returns 500 when provider fails', async () => {
    vi.mocked(MeshyClient).mockImplementation(
      function (this: InstanceType<typeof MeshyClient>) {
        this.createTextToTexture = vi.fn().mockRejectedValue(new Error('Meshy API down'));
      } as unknown as typeof MeshyClient
    );
  ...
  it('calls refundTokens when provider throws and usageId exists', async () => {
    vi.mocked(MeshyClient).mockImplementation(
      function (this: InstanceType<typeof MeshyClient>) {
        this.createTextToTexture = vi.fn().mockRejectedValue(new Error('Meshy down'));
      } as unknown as typeof MeshyClient
    );
