...
export async function GET(request: NextRequest) {
  ...
  const rl = await rateLimit(`user:generate-skybox-status:${authResult.ctx.user.id}`, 60, 60_000);
  ...
  try {
    ...
    // For skybox, we return the first available texture map as the equirectangular image
    const resultUrl = status.maps
      ? Object.values(status.maps)[0]
      : undefined;
