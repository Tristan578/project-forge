...
function makeRequest(body: unknown) {
  return new Request('http://test/api/generate/texture', {
    method: 'POST',
...
describe('POST /api/generate/texture', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const res = await POST(makeRequest({ prompt: 'wood texture' }) as any);
  ...
  it('returns 400 for invalid JSON', async () => {
    const req = new Request('http://test/api/generate/texture', {
      method: 'POST',
      body: 'not json',
    });
  ...
  it('returns 201 on successful texture generation', async () => {
    const res = await POST(makeRequest({ prompt: 'wood texture' }) as any);
