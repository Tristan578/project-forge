...
describe('POST /api/generate/sfx', () => {
  beforeEach(async () => {
    ...
    vi.mocked(ElevenLabsClient).mockImplementation(
      function (this: InstanceType<typeof ElevenLabsClient>) {
        this.generateSfx = vi.fn().mockResolvedValue({ audioBase64: 'base64data', durationSeconds: 5 });
  ...
  it('returns 422 for invalid duration', async () => {
    const res = await POST(makeRequest({ prompt: 'explosion', durationSeconds: 30 }));
    expect(res.status).toBe(422);
    const data = await res.json();
  ...
  it('returns 500 when provider fails', async () => {
    vi.mocked(ElevenLabsClient).mockImplementation(
      function (this: InstanceType<typeof ElevenLabsClient>) {
        this.generateSfx = vi.fn().mockRejectedValue(new Error('ElevenLabs API down'));
      } as unknown as typeof ElevenLabsClient
    );
