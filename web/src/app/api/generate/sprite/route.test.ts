...
function makeRequest(body: unknown) {
  return new Request('http://test/api/generate/sprite', {
    method: 'POST',
...
describe('POST /api/generate/sprite', () => {
  ...
  it('returns 400 for invalid JSON', async () => {
    const req = new Request('http://test/api/generate/sprite', {
      method: 'POST',
      body: 'not json',
    });
  ...
  it('returns 500 when provider fails', async () => {
    vi.mocked(SpriteClient).mockImplementation(function () {
      return {
        generateSprite: vi.fn().mockRejectedValue(new Error('Provider error')),
      } as any;
    } as any);
