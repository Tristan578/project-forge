...
vi.mock('@/lib/generate/spriteClient', () => ({
  SpriteClient: vi.fn(() => ({
    generateTileset: vi.fn().mockResolvedValue({ taskId: 'task-1', status: 'pending' }),
  })),
}));

function makeRequest(body: unknown) {
  return new Request('http://test/api/generate/tileset-gen', {
    method: 'POST',
...
describe('POST /api/generate/tileset-gen', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const res = await POST(makeRequest({ prompt: 'grass tileset', tileSize: 32 }) as any);
  ...
  it('returns 400 for invalid JSON', async () => {
    const req = new Request('http://test/api/generate/tileset-gen', {
      method: 'POST',
      body: 'not json',
    });
  ...
  it('returns 422 for short prompt', async () => {
    const res = await POST(makeRequest({ prompt: 'ab', tileSize: 32 }) as any);
  ...
  it('returns 500 when provider fails', async () => {
    vi.mocked(SpriteClient).mockImplementation(function () {
      return {
        generateTileset: vi.fn().mockRejectedValue(new Error('Replicate down')),
      } as any;
    } as any);
  ...
  it('returns 201 on successful tileset generation', async () => {
    const res = await POST(makeRequest({ prompt: 'grass tileset', tileSize: 32 }) as any);
