...
describe('POST /api/publish', () => {
  ...
  it('returns 401 if unauthenticated', async () => {
    vi.mocked(authenticateRequest).mockResolvedValue({ ok: false, response: NextResponse.json({ error: 'Unauthorized' }, { status: 401 }) });
  ...
  it('returns 422 for blocked content in title', async () => {
    vi.mocked(authenticateRequest).mockResolvedValue({ ok: true, ctx: { clerkId: '123', user: makeUser() } });
    vi.mocked(moderateContent).mockReturnValueOnce({ severity: 'block', reasons: ['bad'], cleaned: '*** game' });
