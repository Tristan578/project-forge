...
describe('InitOverlay', () => {
  ...
  it('shows phase labels', () => {
    vi.mocked(useEngineStatus).mockReturnValue(baseStatus);
    render(<InitOverlay />);
    expect(screen.getByText('Loading WASM (1.5s)')).toBeInTheDocument();
    expect(screen.getByText('Initializing Renderer')).toBeInTheDocument();
    expect(screen.getByText('Setting Up Scene')).toBeInTheDocument();
  });

  it('shows elapsed time', () => {
    vi.mocked(useEngineStatus).mockReturnValue(baseStatus);
    render(<InitOverlay />);
    expect(screen.getByText(/Elapsed: 2\.0s/)).toBeInTheDocument();
  });
  ...
  it('shows Copy Debug Log button in failed state', () => {
    ...
    render(<InitOverlay />);
    expect(screen.getByText('Copy Debug Log')).toBeInTheDocument();
  });
  ...
  it('shows Switch to WebGL2 Mode button in failed state (PF-845)', () => {
    ...
    render(<InitOverlay />);
    expect(screen.getByText('Switch to WebGL2 Mode')).toBeInTheDocument();
  });
