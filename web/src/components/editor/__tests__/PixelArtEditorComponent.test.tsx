...
describe('PixelArtEditor', () => {
  ...
  it('renders the editor when open is true', () => {
    ...
    expect(screen.getByText('Pixel Art Editor')).not.toBeNull();
  });
  ...
  it('renders canvas size selector with default 16x16', () => {
    ...
    expect(screen.getByText('16x16px')).not.toBeNull();
  });
  ...
  it('changes canvas size via select dropdown', () => {
    ...
    expect(screen.getByText('32x32px')).not.toBeNull();
  });
  ...
  it('shows default zoom level', () => {
    render(<PixelArtEditor open={true} onClose={vi.fn()} />);
    expect(screen.getByText('Zoom: 16x')).not.toBeNull();
  });
  ...
  it('zoom in doubles zoom up to 64', () => {
    ...
    expect(screen.getByText('Zoom: 32x')).not.toBeNull();
    fireEvent.click(zoomInBtn);
    expect(screen.getByText('Zoom: 64x')).not.toBeNull();
    // At max — still 64
    fireEvent.click(zoomInBtn);
    expect(screen.getByText('Zoom: 64x')).not.toBeNull();
  });
  ...
  it('zoom out halves zoom down to 2', () => {
    ...
    expect(screen.getByText('Zoom: 8x')).not.toBeNull();
    fireEvent.click(zoomOutBtn);
    expect(screen.getByText('Zoom: 4x')).not.toBeNull();
    fireEvent.click(zoomOutBtn);
    expect(screen.getByText('Zoom: 2x')).not.toBeNull();
    // At min — still 2
    fireEvent.click(zoomOutBtn);
    expect(screen.getByText('Zoom: 2x')).not.toBeNull();
  });
  ...
  it('renders export PNG button', () => {
    render(<PixelArtEditor open={true} onClose={vi.fn()} />);
    expect(screen.getByText('Export PNG')).not.toBeNull();
  });
  ...
  it('shows apply button when entityId is provided', () => {
    ...
    expect(screen.getByText('Apply to Sprite')).not.toBeNull();
  });
