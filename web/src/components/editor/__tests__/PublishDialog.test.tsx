...
describe('PublishDialog', () => {
  ...
  it('renders Publish Game heading when open', () => {
    ...
    expect(screen.getByText('Publish Game')).not.toBeNull();
  });

  it('renders title input with placeholder', () => {
    setupStore({ sceneName: 'My Scene' });
    render(<PublishDialog isOpen={true} onClose={mockOnClose} />);
    expect(screen.getByPlaceholderText('My Awesome Game')).not.toBeNull();
  });

  it('renders URL Slug label', () => {
    setupStore();
    render(<PublishDialog isOpen={true} onClose={mockOnClose} />);
    expect(screen.getByText('URL Slug')).not.toBeNull();
  });

  it('renders Description textarea', () => {
    setupStore();
    render(<PublishDialog isOpen={true} onClose={mockOnClose} />);
    expect(screen.getByPlaceholderText('A brief description of your game')).not.toBeNull();
  });

  it('renders Tags section', () => {
    setupStore();
    render(<PublishDialog isOpen={true} onClose={mockOnClose} />);
    expect(screen.getByPlaceholderText('e.g. platformer, puzzle')).not.toBeNull();
  });
  ...
  it('shows error message when publishError set', () => {
    ...
    render(<PublishDialog isOpen={true} onClose={mockOnClose} />);
    expect(screen.getByText('Slug already taken')).not.toBeNull();
  });
