...
- [#9737](https://github.com/Tristan578/project-forge/pull/9737) [`f62125d`](https://github.com/Tristan578/project-forge/commit/f62125d4ed60ac1e7c75bfb9d3942c85d88fd2bc) Thanks [@Tristan578](https://github.com/Tristan578)! - Stop upstream provider text reaching clients, and put the control on the response path itself ([#9736](https://github.com/Tristan578/project-forge/issues/9736)). The generation clients fold the upstream **response body** verbatim into the thrown error (`Meshy status error (401): <body>`), and twelve routes returned that error's `message` in their response — so on the platform path, where the credential in play is the platform's rather than the user's, a provider that echoes key material in an auth-failure body would hand a platform secret to any signed-in user. Those routes now return fixed, actionable text and send the detail to Sentry; `/api/game/decompose` still returns its safety-filter reason, narrowed by a typed `PromptRejectedError` rather than by a message prefix any upstream error could also produce.
  ...
  **The guarantee is a runtime chokepoint, not a lint rule.** `withEgressGuard` (`web/src/lib/security/egressGuard.ts`) wraps all 102 App Router handlers and, before a response is returned, redacts the body (JSON parsed and re-serialised, other text redacted as text), every response header value, every `Set-Cookie`, and the `Location` of a redirect. That closes the header, cookie and redirect channels structurally — no dataflow analysis is involved. However the body was assembled, it passes through one function. Enforcement that every route is wrapped is a SHAPE check a parser can answer with certainty: `egressGuardCoverage.test.ts` walks every `src/app/**/route.ts` through the TypeScript parser and names any exported HTTP method that is not a `withEgressGuard(...)` call, failing closed on zero files.
  ...
  Two defects the runtime tests found, both real. **Percent-encoding defeated every credential shape**: they are anchored on a word boundary, and in `?e=invalid%20key%20sk-ant-AAA` the character before `sk-ant-` is the `0` of `%20`, so a redirect `Location` and a `Set-Cookie` value — the two channels the guard exists to close structurally — were passing the key through verbatim. `createRedactionPass({ percentAware })` (`web/src/lib/security/redactSecrets.ts`) adds an opt-in percent-aware mode that matches a decoded view and splices the placeholder back into the raw string at mapped offsets, leaving the surrounding encoding byte-for-byte intact; it is off by default, so the Sentry path and the response constructors are unchanged. **Thirteen regex passes per string leaf** cost real time once the guard put redaction on every response; the shapes are now derived into one alternation that preserves their documented precedence, and the environment secret list is derived once per response rather than once per header value. Figures are below, from a committed harness.
  ...
  **The latency figures now come from a committed harness** (`web/scripts/bench-egress-guard.ts`), which prints node version, platform, CPU and env-key count alongside every number — the previous figures had no artifact anywhere in the diff, and were taken on a 13 KB listing rather than on the bodies that actually pay. Measured on Node 24.5.0, win32 x64, Ryzen 7 3800X, 200 iterations after 30 warmup:
  ...
  | body | bytes | added |
  |---|---|---|
  | error envelope | 41 | +0.243 ms |
  | community listing | 10,402 | +0.299 ms |
  | published scene, 400 entities, no escapes (fast path) | 349,983 | +3.044 ms |
  | same scene with JSON escapes and no secret (fast path) | 350,038 | +5.238 ms |
  | same scene with a secret at depth 8 (match path) | 350,051 | +21.561 ms |
  
  Re-measured in one session after the sixth board's fix, alongside a run of the
  same harness at the previous commit so the delta is a comparison rather than two
  numbers from two machines (lessons-learned [#8](https://github.com/Tristan578/project-forge/issues/8)). Both fast paths are unchanged
  within noise (+2.777 / +4.997 before). The MATCH path grew by ~4.8 ms, and it is
  worth naming what bought that: about 3 ms is the fail-closed boundary re-scan of
  the rewritten body, and the rest is the JSON-escape rewrite per string leaf. It
  is paid only by a response that actually carries a credential.
  ...
  **Object and Map KEYS were never redacted**, so a credential in key position survived even on the path the guard had already decided to rewrite: `{"msy_…":"invalid"}` made the scan say MATCH, bought the whole parse/walk/re-serialise round trip, and re-emitted the key unchanged — the lossy path AND the leak, reported as a rewrite. `openContainer` now runs plain-object and `Map` keys through the redactor, and two keys that redact to the same placeholder are disambiguated (`[REDACTED]`, `[REDACTED] (2)`) in source key order rather than one silently overwriting the other.
  ...
  The added scan has a measured cost and it is stated rather than buried: a 350 KB body with **no** backslash is unchanged, and the same body with escapes in one script source roughly doubles, because it is copied once and scanned a second time. Current figures are in the table below. Narrowing that to windows around escape sites is possible and is deliberately not taken — it is a new place for an off-by-one to become a leak. The match path also grew, by key redaction, and is paid only by a response carrying a credential. A byte-identity test on a large realistic body full of escapes and no secret asserts object IDENTITY, so the fix cannot have been "route everything down the slow path".
  ...
  **A SIXTH board found the fifth board's fix had a mirror image, and it is the same defect class in the opposite direction.** Teaching `hasCandidate` to read JSON escapes closed "detected too late". It opened "detected and then emitted anyway": `redactString` was `redactLiteral` + `redactPercentEncoded`, so percent-encoding had a scan AND an index-mapped rewrite while JSON escaping had a scan only. Every text-mode path in the guard — the malformed-JSON fallback in `redactBufferedBody` (which `/api/sentry` reaches, forwarding an arbitrary upstream body under a hard-coded `application/json`), a non-JSON buffered body, a header value, a `Set-Cookie` and `statusText` — set `hasCandidate`, left the fast path, rewrote NOTHING and returned the credential. That is strictly worse than never detecting it: the response pays the whole slow path and leaks anyway, and the guard reports a rewrite that did not happen. Only the JSON-body path was covered, because the parse restores the leaf and the rewrite then sees the credential unescaped — which is why every existing runtime case passed while five channels leaked. `jsonUnescapeView` is now `jsonUnescapeWithMap` and feeds BOTH halves off one implementation, and `redactJsonEscaped` splices view-space matches back into the raw string at mapped offsets exactly as `redactPercentEncoded` does, composing the two index maps for the unescape-then-percent-decode spelling. Scan and rewrite look at the same four views: raw, percent-decoded, unescaped, and both.
  
  **That fix is not trusted on its own, because the agreement between those two halves has now gone quietly false twice.** `withEgressGuard` RE-SCANS what it is about to emit and fails closed when the answer is still "candidate": an envelope value the rewrite could not clear is replaced wholesale with the placeholder, and a body it could not clear becomes the fixed 500. The assertion is the property ("the output no longer matches"), not the adjacent one ("the rewrite changed something") — a rewrite that removed one of two credentials satisfies the second. `egressGuardFailClosed.test.ts` drives that path with a pass that detects everything and rewrites nothing, with a positive control proving the check is not simply always failing, and `redactSecrets.test.ts` sweeps the new direction (`hasCandidate(t)` implies `redactText(t) !== t`) over every escape `JSON.stringify` writes. One decoding ORDER remains uncovered by both halves and is named in KNOWN GAPS rather than left to inference: percent-decoding before JSON-unescaping (`%5Cn`).
  
  **The guard's own failure path was unobservable.** `catch { return guardFailureResponse(); }` recorded nothing — no `captureException`, no log — so a `RedactionBudgetExceededError`, a `clone()` failure or any bug in the walk silently turned a handler's real response into a fixed 500, indistinguishable in production from an application error and capable of running for weeks with user reports as the only symptom. The file argued the opposite rule forty lines earlier for handler throws. Every fail-closed path now reports through `reportGuardFailure` (a `console.error` for the DSN-less local and preview case, plus `captureException`), carrying only values the guard computed, with the whole body wrapped so the reporter cannot become the failure it reports. The budget error is also now driven end to end through `withEgressGuard` on a large, readable, secret-carrying body rather than only at the unit level.
  
  **And the async failure toast promised a refund it could not deliver.** `failJob`'s `userFacing: false` branch said "Your tokens have been refunded" unconditionally, from a call site that runs after `triggerRefund` — which returns without refunding when the job has no `usageId` (BYOK and cache-hit jobs, never charged tokens) and which on failure only queues the refund for a FUTURE session. `retryGuidance.ts`, added by this same change, writes the rule twenty lines away: promising a refund that did not occur is a support ticket. `triggerRefund` now reports whether it refunded and the sentence claims one only when it did; both previously-unreachable branches have tests. The stacked-toast fix did not work either — the dedupe id was `generation-failed-<jobId>`, so N concurrent jobs had N distinct keys and still produced N indefinite red toasts, which is the outcome the id was added to prevent. `toast.ts` states the real mechanism (an id collapses failures with the same MESSAGE), so the key is message-derived and a test drives two jobs failing together. The hook's own two fallbacks (`Generation timed out`, `Generation failed`) now carry `RETRY_GUIDANCE`; the timeout one is the COMMON case, because `lastStatusErrorRef` is cleared on every successful poll.
  
  **Documentation that would have reproduced the defect.** `.claude/skills/generate-route/SKILL.md` still taught `export const POST = createGenerationHandler(...)` and listed five validation commands, none of them `egressGuardCoverage.test.ts` — the only gate that detects an unwrapped route — so an author following it produced a route outside the control, saw green, and shipped. The frontend skill's Next.js reference and `api-middleware-migrate` had the same shape. And `CLAUDE.md`, loaded at every SessionStart, never mentioned the guard at all. All four now carry the required export shape and the gate that enforces it. `/api/bridges/aseprite/execute` told the user to "Check Sentry for details" on both failure paths — an internal tool they cannot open, for a bridge running on their own machine; it now names what they can actually do, matching the sibling `status` route.
  
  **Two citations that a reviewer following them would find false.** This changeset credited `percentAware` to `redactSecretsAll`, an API that does not exist (it is `createRedactionPass({ percentAware })`), and `redactKeys` justified passing non-string keys through by claiming they "carry no text" — false, and demonstrable: `redactSecrets({ m: new Map([[{ token: 'msy_…' }, 'v']]) })` returns the key with the credential intact. The behaviour is right (a key is not walked, and rebuilding one would change what the key IS) and the path is unreachable from `JSON.parse`; the premise was wrong, so it is restated and kept as a named gap. Separately, `noRawResponseInCatchCoverage.test.ts` asked `if (!config.rules?.[RULE_ID])`, and ESLint normalises a DISABLED rule to `[0]` — which is truthy — so an `off` override anywhere would have left the gate green while reporting those files as covered; it now reads the severity and has a negative control proving a disabled file IS reported. `noRawResponseInCatch.test.ts` declared three `responseHelpers` under a comment claiming it matched the flat config's thirteen, so ten sanctioned names had no coverage and an edit to the list could not fail a test; both now import one shared `eslint-rules/no-raw-response-in-catch.options.mjs`.
