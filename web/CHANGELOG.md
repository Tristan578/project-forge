...
- [#10187](https://github.com/Tristan578/project-forge/pull/10187) [`c402fd1`](https://github.com/Tristan578/project-forge/commit/c402fd1c79b0b4e7029d9e39963a46017baa55f2) Thanks [@Tristan578](https://github.com/Tristan578)! - Say when a game-component value was not applied as asked. When the AI chat, an MCP client or the game generator asks for a value the engine will not hold (a platform speed of 99999, a 300-point patrol route), the tool result now lists each adjustment ("Moving Platform speed: you asked for 99999, it was capped at 1000"), the chat card shows it without expanding, generation steps report it as a warning, and the inspector marks the adjusted field with the requested value still readable. Nothing is reported when every value was used as given.

- [#10263](https://github.com/Tristan578/project-forge/pull/10263) [`610b567`](https://github.com/Tristan578/project-forge/commit/610b56712f5ac4f902a7df9c1b47399787d42832) Thanks [@Tristan578](https://github.com/Tristan578)! - Creators can now see how their games are doing. A new Analytics page, reached from the dashboard header, shows how many of your games are live, total plays, your token usage this billing cycle, and plays for each game, most played first. It is backed by a new signed-in `GET /api/creator/stats` endpoint that only ever returns your own numbers.

- [#10258](https://github.com/Tristan578/project-forge/pull/10258) [`6848d27`](https://github.com/Tristan578/project-forge/commit/6848d270b35ff482dab02d1b8d24da5e82c4bbd0) Thanks [@Tristan578](https://github.com/Tristan578)! - The first time an AI build finishes, the editor celebrates it once, and the Getting Started checklist gains a "Build a Game with AI" task that stays ticked afterwards. A build that fails or is cancelled doesn't count, and later builds don't celebrate again. The new task isn't required to unlock the Advanced section, so anyone who already unlocked it keeps it unlocked.

- [#10254](https://github.com/Tristan578/project-forge/pull/10254) [`bb1cf73`](https://github.com/Tristan578/project-forge/commit/bb1cf7350071b47234cde0bbbaccb8a222686786) Thanks [@Tristan578](https://github.com/Tristan578)! - "Make me a game" now asks before it spends build tokens. After you describe your game, "Plan my game" designs it and shows the plan with its estimated token cost. The build starts, and its tokens are taken, only when you press "Build it". "Discard plan" asks once, then drops the plan (designing it again costs tokens), and "Close" keeps it so reopening the dialog brings you back to it. Building from the orchestrator panel works the same way: tokens are taken when the build starts, not while the plan waits. If the server declines to start a build, no build tokens are taken and the plan stays in the review with the reason and what to do next: a link to buy tokens for a short balance, or a plain explanation for an expired session, a rate limit, or an account problem. If the server's answer cannot be confirmed, the build stops and asks you to check your balance before trying again.
...
- [#10199](https://github.com/Tristan578/project-forge/pull/10199) [`aaee823`](https://github.com/Tristan578/project-forge/commit/aaee8231c08bd9712baefe895a633d1cb7ab0335) Thanks [@Tristan578](https://github.com/Tristan578)! - Add a timed, manifest-pinned performance report. The profiler's new Timed capture section and the in-app AI (`capture_performance_report`, `get_performance_report`, `compare_performance_reports`, `set_performance_baseline`, `cancel_performance_capture`) share one implementation: a 10 s warm-up and 60 s capture produce p50/p95/p99 frame time, first-interactive time and memory availability, pinned to the build, fixture checksum, exact browser version, GPU, backend, viewport and cache state, and judged against the versioned `desktop@1` budgets (p95 frame time at most 16.7 ms, cold first interactive at most 5 s). Anything the browser cannot measure is reported as unknown and never passes a budget. A comparison against a baseline from a different browser version or fixture is flagged incompatible instead of claiming an improvement. Reports download as JSON with raw samples.
  
  The performance profiler overlay now follows the editor theme instead of staying dark in every theme. It is built from the shared design-library controls, and its buttons, inputs and selects are 44px tall on mobile.
  ...
  Exported games now send a scene load the engine accepts: the exporters sent `load_scene` as a JSON string immediately after `init_engine`, which the engine refused. They now send `{ json }` once the engine accepts commands, switch 2D projects to the 2D camera so sprites render, and send `set_quality` as an object. The runtime engine build still does not apply scene loads, so an export that carries it shows the default scene until [#10195](https://github.com/Tristan578/project-forge/issues/10195) is fixed; `/play` and exported script commands still send string payloads ([#10196](https://github.com/Tristan578/project-forge/issues/10196)). The profiler no longer reports 0 MB of memory when the browser does not expose the JS heap.

- [#10256](https://github.com/Tristan578/project-forge/pull/10256) [`e5c2667`](https://github.com/Tristan578/project-forge/commit/e5c26675a38c7c7143552280782d5ef1d04d9166) Thanks [@Tristan578](https://github.com/Tristan578)! - Add an opt-in isolation boundary for in-editor game scripts ([#8700](https://github.com/Tristan578/project-forge/issues/8700)). With `NEXT_PUBLIC_SCRIPT_ISOLATION=sandboxed-origin` set at build time, scripts run in the same worker code but inside a hidden `sandbox="allow-scripts"` iframe with its own `connect-src 'none'` policy, so a script that escapes the name shadowing runs in an opaque origin, where it cannot read the app's cookies or storage, and cannot make any network request, whether or not the network globals were revoked first. The default (`revoke`, or any value other than the exact strings `sandboxed-origin` / `ast`) is unchanged. `ast` is reserved and currently runs the sandboxed transport with a plain-language notice in the script console. If the sandbox cannot start, Play stops with a single message chosen by why: when the browser refuses to run the sandboxed worker, the creator is told their scripts can't run in this browser with the current editor settings (and that a different browser might work), with no retry advice, because retrying cannot help; when the editor build shipped without the sandbox's code, the creator is told their scripts can't run in this version of the editor and that the fault is not in their game, again with no retry advice; when the sandbox simply did not answer in time, or its code failed to load, the creator is told to press Play again or reload the editor.
...
- [#10236](https://github.com/Tristan578/project-forge/pull/10236) [`64323cc`](https://github.com/Tristan578/project-forge/commit/64323cc532f14bde988c9fdf65c4c5a32ba559e4) Thanks [@Tristan578](https://github.com/Tristan578)! - Grant every brand-new account a one-time allocation of 50 trial tokens (`TRIAL_GRANT_TOKENS`) on the Clerk `user.created` webhook, and let a free account spend them: while a `starter` account holds spendable tokens every AI gate treats it as `hobbyist`, so chat and the hobbyist generation panels open until the balance is gone. The gates are `/api/chat` and `/api/game/decompose` (`assertAiAccess`), the platform-key resolver, the editor's panel gate, and a new server-side panel gate: every `/api/generate/*` route declares the panel it fronts, and the factory and `voice/batch` refuse a caller whose panel would render locked (`panelTierGateResponse`), so a trial account never reaches a creator-tier generator. Status pollers use `panelTierGateResponseForPoll` instead, which ignores the live balance. A `starter` that has held tokens (`monthlyTokens > 0 || addonTokens > 0`) counts as `hobbyist` however many are left, so a trial user whose one generation spent the whole grant still receives the result, while creator-tier status routes stay refused; a `starter` that never held tokens is refused on hobbyist status routes as before. The status routes do not check that the `jobId` belongs to the caller (pre-existing, tracked in [#10262](https://github.com/Tristan578/project-forge/issues/10262)), so the poll gate is not an ownership check. The resolver skips its tier and balance checks for the matching zero-cost `STATUS_CHECK_OPERATION` call (the pollers and the QStash `generation-complete` callback), because the job was paid for at creation. That also ends a lockout already on `main`: a hobbyist whose last generation left the balance at exactly 0 could not poll it, and the durable callback refunded the job instead of delivering it. The generation poller now shows the status route's `message` sentence instead of a bare code such as `TIER_REQUIRED`. The free plan's bullets now say "50 trial AI tokens at signup" and "No monthly AI tokens". The grant is idempotent at the database (a `trial_grant` row in `credit_transactions` is the arbiter behind the existing partial unique index), never runs on `user.updated`, and a failure is captured in Sentry with `context: trial-token-grant-failure` before the webhook's existing retry path handles it. Adds the P1 alert rule and runbook section 5.9 ([#7715](https://github.com/Tristan578/project-forge/issues/7715)).

### Patch Changes

- [#10212](https://github.com/Tristan578/project-forge/pull/10212) [`2b335ec`](https://github.com/Tristan578/project-forge/commit/2b335ec29599800a2cba4af4d83c7d516d3b27cf) Thanks [@Tristan578](https://github.com/Tristan578)! - In 2D projects the built-in Collectible, Damage Zone, Checkpoint, Teleporter, Trigger Zone and reach-goal Win Condition components now react when the player touches them. The engine's collision tracker only read contacts from the 3D simulation, so a 2D game's contacts never reached those components and every one of them needed a hand-written collision script. The tracker now merges the 2D simulation's contacts into the same per-frame set; 3D behaviour is unchanged.

- [#10255](https://github.com/Tristan578/project-forge/pull/10255) [`1858e7d`](https://github.com/Tristan578/project-forge/commit/1858e7de32a145b7b236735c564820df3f542247) Thanks [@Tristan578](https://github.com/Tristan578)! - The platform Anthropic key can now come from Anthropic Workload Identity Federation instead of the long-lived `ANTHROPIC_API_KEY`. With `ANTHROPIC_WIF_FEDERATION_RULE_ID`, `ANTHROPIC_WIF_ORGANIZATION_ID` and `ANTHROPIC_WIF_SERVICE_ACCOUNT_ID` all set, the server exchanges its Vercel OIDC token for a short-lived Anthropic token and uses it for direct-backend chat and for the localization and pacing generators. Any failed exchange is reported to Sentry and falls back to `ANTHROPIC_API_KEY`. With any of the three unset nothing changes and no extra network call is made. A partial configuration logs a startup warning. Setup: `docs/guides/anthropic-wif-setup.md`.

- [#10276](https://github.com/Tristan578/project-forge/pull/10276) [`5b95fc7`](https://github.com/Tristan578/project-forge/commit/5b95fc73af1170cfe995ba5e2092fa1d6f7cff69) Thanks [@Tristan578](https://github.com/Tristan578)! - The Aseprite bridge can now turn pixel data (a palette plus per-frame grids of colour indices) into a real sprite sheet, instead of blank frames. The data is validated and drawn by a fixed template, so no model- or client-written Lua ever runs. The raw bridge route no longer lets a caller choose where the server reads or writes files: it runs only `createSprite` and `createAnimation`, saves to a server-chosen temp file, and returns that file's bytes (base64) instead of a path. Requests are capped at 2048×2048 pixels and 256 frames, and a saved file over 8 MiB is not returned. `editSprite`, `applyPalette` and `exportSheet` are refused for now, because they need an input sprite the route cannot yet accept safely ([#10283](https://github.com/Tristan578/project-forge/issues/10283)).
...
- [#10219](https://github.com/Tristan578/project-forge/pull/10219) [`fa7b32e`](https://github.com/Tristan578/project-forge/commit/fa7b32e71c862a481aa27999011c48d935480f50) Thanks [@Tristan578](https://github.com/Tristan578)! - Re-baseline the total JS bundle gate to warn at 6 MB and fail at 6.5 MB (was 5.5 / 6) in `performanceTargets.ts` and the `check-bundle-size.js` mirror. `main` measured 6.00 MB at `aaee8231` ([#10199](https://github.com/Tristan578/project-forge/issues/10199)) against the 6 MB hard limit, so the "Check JS bundle size" step failed on the trunk and every open PR inherited the red. The metric counts every emitted chunk, so lazy-loading the new performance panels would not change it; the creep itself is tracked in [#8910](https://github.com/Tristan578/project-forge/issues/8910).

- [#10216](https://github.com/Tristan578/project-forge/pull/10216) [`d5df9d2`](https://github.com/Tristan578/project-forge/commit/d5df9d2d10b1bc543f18bdaea29d17acd0c307a2) Thanks [@Tristan578](https://github.com/Tristan578)! - The in-app AI now reads what each editor tool actually returned. Every tool whose result is an object (`get_game_components`, `get_game_camera`, `add_game_component`, most query tools) was handed back to the model as the literal text `[object Object]`, so query tools returned no information and write tools could not report anything beyond "it ran". Results are now serialised as JSON, strings stay verbatim, and any result longer than 8,000 characters is cut there with a note saying how much was omitted.

- [#10215](https://github.com/Tristan578/project-forge/pull/10215) [`649714c`](https://github.com/Tristan578/project-forge/commit/649714c12b5592b71079145185bb4f005dd35161) Thanks [@Tristan578](https://github.com/Tristan578)! - `npm run db:push` runs its push guard again instead of crashing before it. The guard script ended in a top-level `await`, which tsx's CommonJS output refuses, so the command died at transform time and never reached `drizzle-kit push`, even for the fresh, journal-less databases the guard exists to allow. A subprocess test now runs the script the way npm does, and a sweep checks every tsx-run script compiles as CommonJS.

- [#10210](https://github.com/Tristan578/project-forge/pull/10210) [`4bf8bc3`](https://github.com/Tristan578/project-forge/commit/4bf8bc3beb1fccfdaee70b9a5ed46146dc02a088) Thanks [@Tristan578](https://github.com/Tristan578)! - Opening a saved project in a fresh browser tab now loads its scene. The editor page hands the stored scene to the store before the engine has attached, and that load was deferred and then forgotten, so the tab showed the engine's starter scene and the next autosave could write it over the project. The deferred load is now held and replayed once, through the same path as any other load, the moment the engine attaches; a rejection then locks saving the same way a live rejection does.
...
- [#10243](https://github.com/Tristan578/project-forge/pull/10243) [`d8f3ef5`](https://github.com/Tristan578/project-forge/commit/d8f3ef56686c846415ab8b35ecd32fdf5109da2b) Thanks [@Tristan578](https://github.com/Tristan578)! - Show a generation that finished while the tab was closed as finished ([#8892](https://github.com/Tristan578/project-forge/issues/8892)).
  ...
  - A completed one shows as finishing ("Downloading result...") from the first render rather than appearing done and then spinning, and once imported it gets one confirmation toast ("Your 3D model finished while you were away and was added to your project."). A failed one shows no spinner, only the existing failure message.
...
- [#10217](https://github.com/Tristan578/project-forge/pull/10217) [`0e18a05`](https://github.com/Tristan578/project-forge/commit/0e18a05089d1a7ee4ece6e7354f813e6a970d4e2) Thanks [@Tristan578](https://github.com/Tristan578)! - The editor engine now initializes reliably in development. Under React's development-mode effect replay the engine hook marked itself initialized before the asynchronous WASM load, its cleanup cancelled that load, and the replayed effect saw the mark and returned, so the viewport could sit on "Starting engine..." while the same build booted in production. Initialization now has explicit ownership: an attempt retired before `init_engine` releases its claim so the replay starts the one real attempt (reusing the in-flight download), and an attempt that reached `init_engine` keeps the canvas so nothing can start Bevy twice.
...
- [#10248](https://github.com/Tristan578/project-forge/pull/10248) [`314fe47`](https://github.com/Tristan578/project-forge/commit/314fe4751af693231b0ad5d9431b2d5c99e9ac7a) Thanks [@Tristan578](https://github.com/Tristan578)! - Credit the original creator on remixed games ([#7858](https://github.com/Tristan578/project-forge/issues/7858)).
  
  Forking a published game already recorded where it came from, but nothing read that record back, so a remix looked like an original and the source creator got no credit. The community game detail now reports how many times a game has been forked and, for a game that is itself a fork, which game and creator it came from, with a link to that game's play page. If the original has since been unpublished or removed, the detail says only that the source is no longer available and exposes none of its details. Adds an index on the fork table's forked-project column for the lookup.
...
- [#10188](https://github.com/Tristan578/project-forge/pull/10188) [`43376dc`](https://github.com/Tristan578/project-forge/commit/43376dc4d17b3799811ef76ed08f53b4bd6b971a) Thanks [@Tristan578](https://github.com/Tristan578)! - Give every colour picker, slider and dropdown in the editor an accessible name. Scene Settings, the light and material inspectors, and the export, generation and physics panels now pair each control with its visible label, so assistive technology reads the label as the control's name and clicking a label focuses its control. Each material texture slot's Upload button now names the slot it fills. Scene Settings is back under the accessibility audit with no exemption.
...
- [#10224](https://github.com/Tristan578/project-forge/pull/10224) [`4b59add`](https://github.com/Tristan578/project-forge/commit/4b59add10f6ebd203ad5d0e6adad7937b84561d1) Thanks [@Tristan578](https://github.com/Tristan578)! - The welcome wizard's "Start from Template" path now loads the chosen starter game through the editor store, completes onboarding only once the load has landed, and stays open with the store's error when it fails, instead of completing onboarding onto a blank scene. Its Platformer and Runner cards read their 2D/3D tag from the template registry (both are 3D games, not 2D). The "Build with AI" card follows the user's plan as it loads: it shows a pending state until the profile resolves, then the enabled or locked card, rather than a locked "Upgrade" card for a paying user on first render ([#10156](https://github.com/Tristan578/project-forge/issues/10156)).

- [#10214](https://github.com/Tristan578/project-forge/pull/10214) [`6ea14be`](https://github.com/Tristan578/project-forge/commit/6ea14bec8e8512d345bdb2bd7812f372bdbb47f0) Thanks [@Tristan578](https://github.com/Tristan578)! - `update_game_component` (in-app AI and MCP) is now the partial update its description promised: only the named properties change and every other property keeps its current value. It used to rebuild the component from the named properties alone, so asking to make a moving platform faster also reset its route, loop mode and pause to the defaults. Updating a component the entity does not have now reports that instead of reporting success, and the `componentType` enum in the command manifest gains `dialogue_trigger`, which the engine and the handler already accepted.

- [#10226](https://github.com/Tristan578/project-forge/pull/10226) [`14bd6fc`](https://github.com/Tristan578/project-forge/commit/14bd6fc41847ff3be7ffd83cce851651c52a1495) Thanks [@Tristan578](https://github.com/Tristan578)! - Load the engine on `/play` from the engine CDN ([#7580](https://github.com/Tristan578/project-forge/issues/7580)).
  
  The public play page hardcoded the same-origin `/engine-pkg-*` path and never consulted `NEXT_PUBLIC_ENGINE_CDN_URL`, so every player downloaded the WASM engine through the Vercel origin while the editor already used the CDN. The play loader now resolves the same ordered list the editor does — the versioned CDN prefix when `NEXT_PUBLIC_ENGINE_VERSION` is set, `/latest/` otherwise, then same-origin — and falls through to the next origin when a load fails or its glue import stalls past a per-origin deadline (the binary download keeps the page's global budget, so slow links are not cut short), so a CDN outage or a stalled CDN degrades to today's behaviour rather than a broken page. A skipped origin is reported to Sentry from the play page so a misconfigured CDN is visible rather than silently slow. With no CDN configured nothing changes.
...
- [#10211](https://github.com/Tristan578/project-forge/pull/10211) [`ede1c26`](https://github.com/Tristan578/project-forge/commit/ede1c26bb349e636e66ac9bd0d02e5a69dffaf15) Thanks [@Tristan578](https://github.com/Tristan578)! - Adding two game components to the same entity in one engine frame now keeps both. The engine inserted a missing component set through deferred commands, so the second add in the same frame still saw none, built a fresh set holding only itself, and that second insert replaced the first: a player given a Character Controller and Health in one step (as the in-app AI's compound tools do) kept only Health, and a collect-all or reach-goal scene then failed its pre-play check with no player. Pending inserts are now staged per entity and applied once, and the change event for each add lists the accumulated set.
...
- [#10247](https://github.com/Tristan578/project-forge/pull/10247) [`20a5647`](https://github.com/Tristan578/project-forge/commit/20a56478baa152ad7a240193d3bf0e199fb69fa5) Thanks [@Tristan578](https://github.com/Tristan578)! - Keep the AI chat's prompt cache warm across scene edits on the premium model ([#8859](https://github.com/Tristan578/project-forge/issues/8859)).
...
- Updated dependencies [[`33116b2`](https://github.com/Tristan578/project-forge/commit/33116b25de2fe3f65b42720ffb278d2f10808b7d), [`7f4653b`](https://github.com/Tristan578/project-forge/commit/7f4653b46a0a10d16ec4b80095c7321a7c506540), [`d0f8cbc`](https://github.com/Tristan578/project-forge/commit/d0f8cbc915e7ffc579e09fbf03a88bacf435cf67)]:
  - @spawnforge/ui@0.3.1
