...
OUT="$(EXPECT_COMMIT="${DEPLOYED_SHA:0:8}" e2e 200 "$INDEX_OK" 200 "$CATEGORY_OK")"; RC=$?
if [ "$RC" = 0 ]; then
  pass "the commit is compared on its first 8 chars (an abbreviated expected SHA matches a full stamp)"
else
