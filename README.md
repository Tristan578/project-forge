...
| Metric | Value |
|--------|-------|
| MCP commands | 360 across 41 categories (290 public), measured 2026-09-15 with `jq '.commands \| length' mcp-server/manifest/commands.json` — per-category status in the [capability matrix](docs/capability-matrix.md) |
...
```
MCP Server (360 commands, 41 categories)       AI agents + LLM tool use
    |  JSON commands
...
│   ├── manifest/commands.json  # 360 commands across 41 categories (see docs/capability-matrix.md)
│   └── src/
