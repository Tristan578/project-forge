...
| Metric | Value |
|--------|-------|
| MCP commands | 369 across 41 categories (299 public), measured 2026-09-15 with `jq '.commands \| length' mcp-server/manifest/commands.json` — per-category status in the [capability matrix](docs/capability-matrix.md) |
...
```
MCP Server (369 commands, 41 categories)       AI agents + LLM tool use
    |  JSON commands
...
│   ├── manifest/commands.json  # 369 commands across 41 categories (see docs/capability-matrix.md)
│   └── src/
