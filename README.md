...
| Metric | Value |
|--------|-------|
| MCP commands | 358 across 41 categories (286 public), measured 2026-09-05 with `jq '.commands \| length' mcp-server/manifest/commands.json` — per-category status in the [capability matrix](docs/capability-matrix.md) |
...
- **Prefab System** — Reusable entity templates with 8+ built-in prefabs, import/export, search, and saved-link metadata inspection. Linked scene placement, nesting, and propagation remain unavailable ([#9811](https://github.com/Tristan578/project-forge/issues/9811)).
...
```
MCP Server (358 commands, 41 categories)       AI agents + LLM tool use
    |  JSON commands
...
│   ├── manifest/commands.json  # 358 commands across 41 categories (see docs/capability-matrix.md)
│   └── src/
