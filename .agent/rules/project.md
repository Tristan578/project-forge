# SpawnForge — Antigravity Project Rules
...
Antigravity does not auto-execute hooks. You MUST run these manually:

```bash
# At session start (starts taskboard, pulls GitHub sync, shows backlog)
cd project-forge && bash .claude/hooks/on-session-start.sh

# After completing work (worktree safety commit + GitHub push)
cd project-forge && bash .claude/hooks/on-stop.sh

# After editing web/ files (ESLint on changes)
