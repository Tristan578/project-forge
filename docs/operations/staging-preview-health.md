...
Create a Protection Bypass for Automation key on spawnforge-staging only and store it as the GitHub repository secret VERCEL_AUTOMATION_BYPASS_STAGING. Keep it out of command arguments, files, URLs and logs. Provision through protected input or the GitHub settings UI. When rotating, store the new secret before revoking the old project key. Keep Deployment Protection enabled.
