...
|-----|--------------|----------|
| Provider-key provisioning needs verification | Platform generation requires its configured provider keys; creators can instead add supported Meshy, ElevenLabs, OpenAI or Replicate BYOK keys in Settings → API Keys. Sprite generation uses OpenAI for DALL-E or Replicate for SDXL, with a separate remove.bg key for supported background removal. Current production provisioning must be verified under the linked issue. | [#9117](https://github.com/Tristan578/project-forge/issues/9117) |
...
| Background removal is limited to new DALL-E sprites | Sprite generation honours `removeBackground` on DALL-E when a remove.bg BYOK or platform key is configured. Missing keys or lookup errors keep the original image and warn; remove.bg request failures refund the generation. SDXL reports removal unsupported. Standalone removal of an existing asset is unavailable and starts no generation. | [#9734](https://github.com/Tristan578/project-forge/issues/9734) |
