...
| 2D mesh-attachment deformation is authored but unproven | The SkeletonInspector now has a mesh-attachment editor (vertices + per-vertex bone weights, with the same unknown-bone and zero-total-weight validation as `add_skeleton2d_mesh_attachment`), so mesh attachments no longer need command or chat (see 2D Subsystem below). What remains is real-runtime evidence: that an editor-authored mesh actually deforms with the bones and round-trips through save/reopen and the manual/AI boundary. | [#10005](https://github.com/Tristan578/project-forge/issues/10005) (residual of [#9732](https://github.com/Tristan578/project-forge/issues/9732)) |
