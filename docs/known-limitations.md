...
| Music generation cannot be provisioned | `PLATFORM_SUNO_KEY` cannot be obtained and BYOK Suno keys cannot be either, so music is unavailable through every entry point. [#9725](https://github.com/Tristan578/project-forge/pull/9725) merged on 2026-09-06 as `c0c4eaff`, so `generate_music` is refused before any token is deducted and is withheld from the model's tool set (pinned by `web/src/lib/ai/__tests__/agentToolAvailability.test.ts`), and the dialog entry points are gated. The capability is unchanged: no Suno key can be obtained through any path, so music stays unavailable everywhere. | [#9522](https://github.com/Tristan578/project-forge/issues/9522) |
...
| Adaptive-music intensity: script path only | The AdaptiveMusicInspector slider now forwards to `audioManager.setMusicIntensity('default', clamped)` (the same path as the chat tool `set_music_intensity`), so the UI and chat both reach the audio engine (#9735). The script call remains a phantom (#9284). | [#9284](https://github.com/Tristan578/project-forge/issues/9284) |
...
- Adaptive music with stem mixing and intensity control — the inspector slider and the chat tool `set_music_intensity` both reach the audio engine ([#9735](https://github.com/Tristan578/project-forge/issues/9735)); the script call is a phantom ([#9284](https://github.com/Tristan578/project-forge/issues/9284))
