...
|-----|--------------|----------|
| Provider-key provisioning needs verification | Platform generation requires its configured provider keys; creators can add supported Meshy or ElevenLabs BYOK keys in Settings → API Keys. Settings has no OpenAI, Replicate or remove.bg key option. New sprite generation therefore needs OpenAI (DALL-E) or Replicate (SDXL) platform provisioning, plus a separate remove.bg platform key for supported background removal. Current production provisioning must be verified under the linked issue. | [#9117](https://github.com/Tristan578/project-forge/issues/9117) |
| Image generation has no product entry point | The registered `image` capability has no consuming authoring route; `generate_skybox` uses Meshy under `texture` and `apply_style_transfer` calls no provider. Adding an image-provider key does not create an entry point. | [#9818](https://github.com/Tristan578/project-forge/issues/9818) |
| Background removal is limited to new DALL-E sprites | Sprite generation honours `removeBackground` on DALL-E when a remove.bg platform key or existing stored provider credential is configured. Missing keys or lookup errors keep the original image and warn; remove.bg request failures refund the generation. SDXL reports removal unsupported. Standalone removal of an existing asset is unavailable and starts no generation. | [#9734](https://github.com/Tristan578/project-forge/issues/9734) |
| External MCP is local-only, and allowlisted | `NEXT_PUBLIC_MCP_BRIDGE` is absent from the production build, so `mcpBridgeEnabled()` is false and no external MCP client can attach to the production editor. A local build, or a production build made with the flag, can attach after the in-tab consent prompt — to 322 of the 380 commands: `web/src/lib/mcp/bridgeAllowlist.ts` withholds the `scripting`, `generation`, `export`, `publishing`, `security` and `economy` categories and the `ai:generate` / `project:manage` scopes by design. That local path has not been verified end to end. | [#9722](https://github.com/Tristan578/project-forge/issues/9722) |
...
| Linked prefab editing is unavailable | The Prefabs panel inspects saved link metadata only. Linked entity placement, recursive nesting, and propagation are not implemented; their UI actions are disabled, the three compatibility commands return explicit errors and are withheld from both model tool sets. `list_prefab_instances` inspects metadata through the panel and local MCP; it does not verify viewport entities and is not offered to in-app chat. Flat prefab copies remain available. | [#9811](https://github.com/Tristan578/project-forge/issues/9811) |
