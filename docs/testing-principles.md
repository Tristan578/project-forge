...
- **The curated set is TWO flows, and it stays small on purpose.** Software rendering puts every frame on the CPU, so the job's budget is spent on boot and round-trips rather than on assertions:
...
- **Per-spec time budget:** the config's default per-test cap is `E2E_TIMEOUT_ENGINE_FULL_MS` (90s); `pipeline-live-engine.spec.ts` raises its own describe block to `E2E_TIMEOUT_PIPELINE_LIVE_MS` (180s) because it pays for a cold WASM boot AND dozens of pipeline dispatches. Both constants live in `web/e2e/constants.ts` — **no spec may inline a timeout literal.** A third curated flow needs a written justification for the added minutes, not just a tag; prefer extending one of the two above.
