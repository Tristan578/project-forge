...
- **Tag a curated per-PR engine flow with `@engine-smoke`** — this tag selects the specs the `test-e2e-engine-smoke` job runs (`web/playwright.engine.config.ts`), the only per-PR job that boots the real WASM engine. It uses ANGLE/SwiftShader software WebGL2 (`--use-gl=angle --use-angle=swiftshader-webgl --enable-unsafe-swiftshader`) — NEVER `--disable-gpu`, which leaves wgpu with no GL context and hangs `init_engine()`. GAP: software WebGL2 does NOT cover WebGPU or real-GPU rendering correctness — that still needs a GPU runner. See `docs/audits/2026-05-30-security-testing-audit.md` (F10).
- **The curated set is THREE flows, and it stays small on purpose.** Software rendering puts every frame on the CPU, so the job's budget is spent on boot and round-trips rather than on assertions:
  - `e2e/tests/engine-smoke.spec.ts` — load → spawn → play → export. The breadth check: does the engine come up and answer at all.
  - `e2e/tests/pipeline-live-engine.spec.ts` — runs the real game-creation pipeline, clicks the real Play button, and asserts `engineMode === 'play'` through a genuine engine round-trip, plus the negative case where the winnability gate refuses. The depth check: does a *generated* game actually reach Play.
  - `e2e/engine/referenceGames.spec.ts` (#10159) — builds each versioned reference game (`e2e/fixtures/reference-games/`) through real engine commands, compares the engine's own `export_scene` answer and scene graph with the fixture's expected-state record, and presses Play. The input check: every release journey that plays a reference game (#10152, #10163) depends on that game loading exactly as recorded, so a fixture the engine no longer accepts fails here, per PR, before it can fail a slower journey. Justification for the added time: two tests, each paying one cold boot plus about a dozen export round-trips; measured locally at 11–26 s per test (Windows 11, locally built WebGL2 engine, SwiftShader, `next start` production build, other workloads on the machine). The two games are separate tests rather than one, so a broken 2D fixture cannot hide the 3D result.
- **Per-spec time budget:** the config's default per-test cap is `E2E_TIMEOUT_ENGINE_FULL_MS` (90s); `pipeline-live-engine.spec.ts` raises its own describe block to `E2E_TIMEOUT_PIPELINE_LIVE_MS` (180s) because it pays for a cold WASM boot AND dozens of pipeline dispatches. Both constants live in `web/e2e/constants.ts` — **no spec may inline a timeout literal.** A fourth curated flow needs a written justification for the added minutes, not just a tag; prefer extending one of the three above.
